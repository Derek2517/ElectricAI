...
#Look for filename... in this example if the file is /var/log/requsts.log then proceed; Change Filename to suit correct naming scheme.    
	...
	then
#Counting requests by first using grep to remove all lines that begin with '/' then using awk to sort through what is left and grab the URL and request count, using sed to flip output to left to right.     
